/* Mudae Organizer Rebuild V2 - hibernation manager
   Pauses non-essential visual/image work while the tab/window is not active.
   On return, performs a light harem wake-up: no full render unless explicitly needed.
*/
(function(){
  'use strict';
  ...
  function pauseWork(reason){
    ...
    if (criticalLoading) {
      // v2.598: do not cancel/suspend the initial Loading harem image gate.
      // The boot overlay exists specifically to finish this work before the page
      // becomes interactive; cancelling it on blur made End/Home jumps miss cards.
  ...
  function lightWake(reason){
    ...
    const run = () => {
      ...
      if (!criticalLoading) {
        ...
        // Ask the virtual board to verify its current window only. This should not
        // call full renderAll, rebuild the harem, or re-open modals.
