/* v2.572: Graphics Settings LAB parity bridge.
   Keeps the promoted Graphics menu using the same transparency behavior as the LAB,
   while preserving the original page background/image logic. */
(() => {
  'use strict';
  ...
  const applyPromotedTransparency = (config = readVisualConfig()) => {
    ...
    // Keep compatibility with older app CSS that still reads these variables.
  ...
  const init = () => {
    // If Graphics was moved into body by the settings module, keep the promoted scope intact.
