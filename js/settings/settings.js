...
(function(){
  ...
  const ids = {
    ...
    groupNamingMode: 'groupNamingModeBtn',
  ...
  function els(){
    return {
      ...
      groupNamingMode: $(ids.groupNamingMode),
  ...
  function syncGroupNamingMode(){
    const { groupNamingMode } = els();
    if (!groupNamingMode) return;

    const mode = window.MUDAE_REBUILD_V1?.getGroupNamingMode?.() || 'automatic';
    const isManual = mode === 'manual';
    groupNamingMode.textContent = isManual ? 'Manual' : 'Automatic';
    groupNamingMode.classList.toggle('is-on', isManual);
    groupNamingMode.classList.toggle('is-off', !isManual);
    groupNamingMode.title = isManual
      ? 'Manual group naming is enabled. New groups will ask for a name.'
      : 'Automatic group naming is enabled. New groups use generated names.';
    groupNamingMode.setAttribute('aria-pressed', isManual ? 'true' : 'false');
  }

  function toggleGroupNamingMode(){
    const current = window.MUDAE_REBUILD_V1?.getGroupNamingMode?.() || 'automatic';
    const next = current === 'manual' ? 'automatic' : 'manual';
    window.MUDAE_REBUILD_V1?.setGroupNamingMode?.(next);
    syncGroupNamingMode();
  }
  ...
  function applyBackgroundUrl(){
    ...
    // Keep whatever preset/tint (default/purple/blue) is currently active.
    // Without this, normalizeBackgroundConfig() falls back to 'default'
    // whenever preset isn't explicitly passed in, which is why setting a
    // custom background used to silently revert the theme to default.
    const current = readStoredBackground();
    setCustomBackground({ kind: 'url', value: rawValue, preset: current?.preset || 'default' });
  }
  ...
  function bind(){
    const { gear, graphicsBtn, graphicsBackdrop, graphicsClose, performance, groupNamingMode } = els();
    ...
    if (groupNamingMode) {
      groupNamingMode.addEventListener('click', toggleGroupNamingMode);
    }
    ...
    window.addEventListener('mudae:group-naming-mode-change', syncGroupNamingMode);
