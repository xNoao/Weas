/* Mudae Organizer Rebuild GIF controller.
   GIFs are paused by default. Paused GIFs display a canvas poster captured
   from the first loaded frame, and GIF animation only runs when sufficiently visible.
*/
(function(){
  'use strict';
  ...
  let scanRaf = 0;
  let gifControlSuspendedUntil = 0;
  let gifControlResumeTimer = 0;
  ...
  function isGalleryImage(img){
    return !!(img && (
      img.classList?.contains?.('gallery-gif-live') ||
      img.closest?.('#galleryPanel, .gallery-panel, #galleryGrid, .gallery-grid')
    ));
  }
  ...
  function isGifImage(img){
    return !!(
      ...
      isAnimatedUrl(img.dataset.animatedSrc) &&
      !isGalleryImage(img)
    );
  }
  ...
  function getSuspendRemaining(){
    return Math.max(0, Number(gifControlSuspendedUntil || 0) - nowMs());
  }

  function isSuspended(){
    return getSuspendRemaining() > 0;
  }

  function queueResumeCheck(){
    const remaining = getSuspendRemaining();
    if (!remaining) return false;

    clearTimeout(gifControlResumeTimer);
    gifControlResumeTimer = setTimeout(() => {
      gifControlResumeTimer = 0;
      scheduleCheck();
      if (pendingScanNodes.length) flushPendingScans();
    }, Math.min(1500, remaining + 40));

    return true;
  }

  function suspend(ms = 700, reason = 'suspend'){
    const duration = Math.max(0, Number(ms) || 0);
    if (!duration) return;

    gifControlSuspendedUntil = Math.max(gifControlSuspendedUntil, nowMs() + duration);
    queueResumeCheck();

    try {
      document.documentElement.classList.add('mhp-gif-control-suspended');
      document.body?.classList?.add?.('mhp-gif-control-suspended');
      clearTimeout(suspend._classTimer);
      suspend._classTimer = setTimeout(() => {
        if (isSuspended()) return queueResumeCheck();
        document.documentElement.classList.remove('mhp-gif-control-suspended');
        document.body?.classList?.remove?.('mhp-gif-control-suspended');
      }, duration + 80);
    } catch (_) {}
  }
...
function hidePoster(img){
  ...
  function isMostlyBlankCanvas(canvas) {
    try {
      ...
      for (let i = 0; i < data.length; i += 4) {
        ...
        // Treat transparent or almost-white pixels as blankish.
      ...
      // Cross-origin canvases may be unreadable. In that case we cannot judge,
      // so do not reject the capture.
      return false;
    }
  }
  ...
  async function capturePosterFrame(temp, canvas, attempt = 0) {
    ...
    // Let the browser actually paint/decode the first GIF frame.
  ...
  function ensurePoster(img){
    ...
    temp.onload = async () => {
      try {
        ...
        img.dataset.posterLoading = '0';
        // If capture fails, keep the fallback image and let the GIF play only when visible.
      }
    };
...
function pause(img){
    ...
    // Stop animation only after the poster is covering it. This avoids showing
    // the transparent fallback/white flash.
...
function play(img){
    ...
    // Keep the poster visible until the animated GIF has actually loaded.
    // This prevents the white/blank flash during src swap.
    showPoster(img);
    ...
    const onError = () => {
      img.removeEventListener('load', onLoad);
      img.removeEventListener('error', onError);
      // If GIF fails, keep the poster visible instead of showing a blank image.
      showPoster(img);
    };
  ...
  function checkImage(img){
    ...
    // Hysteresis:
    // - Start only when nearly fully visible.
    // - Once playing, keep playing during normal scroll until it is clearly leaving view.
    if (isPlaying) {
  ...
  function observe(img){
    ...
    // Default: paused with poster/fallback.
    pause(img);
    scheduleCheck();
  }
  ...
  function releaseAll(){
    ...
    document.querySelectorAll?.('img[data-animated-src]').forEach(img => {
      if (!isGalleryImage(img)) release(img);
    });
  }
  ...
  function refresh(){
    ...
    // Gallery panels can be rendered while hidden and become visible right after.
    // Re-check a few times so GIFs start without needing a click.
