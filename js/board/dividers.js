...
(function(){
  ...
  function bindDelegatedDividerActions() {
    ...
    const handle = event => {
      ...
      } else if (action === 'gallery-check') {
        const c = requireCtx();
        const divider = byId(dividerId);
        window.MudaeGalleryUpdate?.startDividerGalleryCheck?.({
          dividerId,
          title: divider?.title || '',
          kind: divider ? c.getDividerCounterKind(divider) : '',
          characters: collectDividerCharacters(dividerId)
        });
      } else if (action === 'move-up') {
        moveDividerSection(dividerId, 'up');
      } else if (action === 'move-down') {
        moveDividerSection(dividerId, 'down');
      }
    };
  ...
  function refreshDividerCollapseView({ anchorDividerId = null, preserveView = false, message = '', alignExpandedDivider = false } = {}) {
    const c = requireCtx();
    ...
    const dividerSelector = anchorDividerId
      ? `.divider-row[data-id="${safeEscape(anchorDividerId)}"]`
      : '';
    const dividerNodeBefore = dividerSelector ? board?.querySelector?.(dividerSelector) : null;
    const dividerTopBefore = dividerNodeBefore?.getBoundingClientRect?.().top;

    const getDividerStickyFocusOffset = () => {
      const candidates = [
        document.querySelector('.topbar'),
        document.querySelector('.app-topbar'),
        document.querySelector('header'),
        document.getElementById('floatingBar'),
        document.getElementById('compactStickyDividerBar')
      ];
      ...
      candidates.forEach(node => {
        ...
        if (rect.height > 0 && rect.top <= 140) {
          top = Math.max(top, Math.round(rect.bottom) + 10);
        }
      });

      return Math.min(190, Math.max(82, top));
    };

    const scrollExpandedDividerToStickyPoint = () => {
      if (!alignExpandedDivider || !dividerSelector) return false;
      const nextNode = c.els?.board?.querySelector?.(dividerSelector);
      if (!nextNode) return false;

      try { window.MHPAutoScrollGuard?.mark?.(1400, { force: true }); } catch (_) {}
      ...
      const maxScroll = c.getDocumentMaxScrollTop?.() ?? Math.max(0, (document.documentElement?.scrollHeight || 0) - (window.innerHeight || 800));
      const desired = Math.max(0, Math.min(maxScroll, current + rect.top - getDividerStickyFocusOffset()));
    ...
    // Keep the first visual update fast, then sync the virtual controller on the next frame.
    c.renderAll({ preserveView });
    c.saveLocal();

    const compensateDividerScroll = () => {
      if (alignExpandedDivider && scrollExpandedDividerToStickyPoint()) return;
    ...
    if (message) c.notifyAppMessage(message);
    return true;
  }
  ...
  function toggleDividerCollapsed(dividerId) {
    ...
    const wasCollapsed = !!divider.collapsed;
    divider.collapsed = !divider.collapsed;

    return refreshDividerCollapseView({
      anchorDividerId: dividerId,
      preserveView: false,
      alignExpandedDivider: wasCollapsed && !divider.collapsed,
      message: `${divider.title || 'Divider'} ${divider.collapsed ? 'minimized' : 'restored'}.`
    });
  }


  function findDividerIndexById(dividerId) {
    const c = requireCtx();
    const list = getState().characters || [];
    return list.findIndex(item => c.isDivider(item) && item.id === dividerId);
  }

  function getParentDividerIndexForScope(startIndex) {
    const c = requireCtx();
    const list = getState().characters || [];
    const divider = list[startIndex];
    const level = getDividerLevel(divider);

    if (!c.isDivider(divider) || level <= 1) return -1;
  ...
  function setScopedSubDividersCollapsed(dividerId, collapsed = true) {
    const c = requireCtx();
    const list = getState().characters || [];
    const startIndex = findDividerIndexById(dividerId);

    if (startIndex < 0) return false;

    const parentIndex = getParentDividerIndexForScope(startIndex);

    if (parentIndex < 0) {
      return setAllDividersCollapsed(collapsed, dividerId);
    }

    const parent = list[parentIndex];
    const parentLevel = getDividerLevel(parent);
    const endIndex = getDividerSectionEndIndex(parentIndex);
    const nextValue = !!collapsed;
    let total = 0;
    let changed = 0;

    for (let i = parentIndex + 1; i < endIndex; i++) {
      const item = list[i];
      if (!c.isDivider(item)) continue;
      if (getDividerLevel(item) <= parentLevel) continue;

      total += 1;
      if (!!item.collapsed !== nextValue) {
        item.collapsed = nextValue;
        changed += 1;
      }
    }

    if (!total) {
      c.notifyAppMessage('This divider has no sub-dividers.');
      return true;
    }

    if (!changed) {
      c.notifyAppMessage(nextValue
        ? 'All sub-dividers in this divider are already minimized.'
        : 'All sub-dividers in this divider are already restored.');
      return true;
    }
    ...
    return refreshDividerCollapseView({
      anchorDividerId: dividerId,
      preserveView: false,
      alignExpandedDivider: !nextValue,
      message: nextValue
        ? `Minimized ${changed} sub-divider${changed === 1 ? '' : 's'} in ${parent.title || 'this divider'}.`
        : `Restored ${changed} sub-divider${changed === 1 ? '' : 's'} in ${parent.title || 'this divider'}.`
    });
  }

  function setAllDividersCollapsed(collapsed = true, anchorDividerId = null) {
    const c = requireCtx();
    const list = getState().characters;
    let changed = 0;
    let total = 0;
    ...
    if (!changed) {
      c.notifyAppMessage(collapsed ? 'All dividers are already minimized.' : 'All dividers are already restored.');
      return true;
    }

    return refreshDividerCollapseView({
      anchorDividerId,
      preserveView: !anchorDividerId,
      alignExpandedDivider: !!anchorDividerId && !collapsed,
      message: collapsed
        ? `Minimized ${changed} divider${changed === 1 ? '' : 's'}.`
        : `Restored ${changed} divider${changed === 1 ? '' : 's'}.`
    });
  }
  ...
  function getDividerSiblingIndexes(dividerId) {
    const c = requireCtx();
    const list = c.app.state.characters || [];
    const index = list.findIndex(item => c.isDivider(item) && item.id === dividerId);

    if (index < 0) {
      return { index: -1, previous: -1, next: -1, end: -1, level: 1 };
    }

    const divider = list[index];
    const level = getDividerLevel(divider);
    const end = getDividerSectionEndIndex(index);
    ...
    for (let i = index - 1; i >= 0; i--) {
      ...
      const itemLevel = getDividerLevel(item);
    ...
    if (end < list.length && c.isDivider(list[end]) && getDividerLevel(list[end]) === level) {
      next = end;
    }

    return { index, previous, next, end, level };
  }


  function moveDividerSection(dividerId, direction) {
    const c = requireCtx();
    const list = c.app.state.characters || [];
    const info = getDividerSiblingIndexes(dividerId);

    if (info.index < 0) return false;

    const divider = list[info.index];
    const title = c.str(divider.title || (info.level > 1 ? 'Sub-divider' : 'Divider')).trim() || (info.level > 1 ? 'Sub-divider' : 'Divider');
    const movingDown = direction === 'down';

    if ((!movingDown && info.previous < 0) || (movingDown && info.next < 0)) {
      c.notifyAppMessage(`${title} cannot move ${movingDown ? 'down' : 'up'} any further.`);
      return false;
    }

    const visualAnchor = c.captureBoardVisualAnchor();
    const blockLength = info.end - info.index;
    const block = list.slice(info.index, info.end);

    if (!movingDown) {
      list.splice(info.index, blockLength);
      list.splice(info.previous, 0, ...block);
    } else {
      const nextEnd = getDividerSectionEndIndex(info.next);
      list.splice(info.index, blockLength);
      list.splice(nextEnd - blockLength, 0, ...block);
    }
    ...
    requestAnimationFrame(() => {
      window.MudaeBoardController?.updateEntriesFromApp?.();
      c.restoreBoardVisualAnchor(visualAnchor, { attempts: 8 });
    });

    c.notifyAppMessage(`${info.level > 1 ? 'Sub-divider' : 'Divider'} moved ${movingDown ? 'down' : 'up'}.`);
    return true;
  }
  ...
  function renderDivider(divider) {
    ...
    levelEl.title = kind === 'subdivider'
      ? `Sub-divider #${c.fmt(counter)} · Board item #${c.fmt(divider.boardIndex || 0)}`
      : `Divider #${c.fmt(counter)} · Board item #${c.fmt(divider.boardIndex || 0)}`;
    ...
    collapseBtn.title = isCollapsed
      ? (level > 1 ? 'Restore this sub-divider scope. Shift + click restores the sub-dividers inside this divider.' : 'Restore this divider section. Shift + click restores all dividers.')
      : (level > 1 ? 'Minimize this sub-divider scope. Shift + click minimizes the sub-dividers inside this divider.' : 'Minimize this divider section like a window. Shift + click minimizes all dividers.');
    ...
    const runCollapseToggle = event => {
      event?.preventDefault?.();
      event?.stopPropagation?.();
      collapseHandledAt = Date.now();

      if (event?.shiftKey) {
        const shouldCollapse = !divider.collapsed;
        if (level > 1) {
          setScopedSubDividersCollapsed(divider.id, shouldCollapse);
        } else {
          setAllDividersCollapsed(shouldCollapse, divider.id);
        }
        return;
      }
    ...
    collapseBtn.addEventListener('click', event => {
      ...
      if (Date.now() - collapseHandledAt < 650) return;
      runCollapseToggle(event);
    });
    ...
    hiddenEl.textContent = isCollapsed
      ? `${c.fmt(summary.characters)} hidden${summary.dividers ? ` · ${c.fmt(summary.dividers)} sub-dividers` : ''}`
      : '';
    hiddenEl.hidden = !isCollapsed;

    const siblingMoveInfo = getDividerSiblingIndexes(divider.id);
    const moveKindLabel = kind === 'subdivider' ? 'sub-divider' : 'divider';
    const moveTitleLabel = divider.title || '';

    const moveUpBtn = document.createElement('button');
    moveUpBtn.type = 'button';
    moveUpBtn.className = 'divider-action-btn divider-move-btn divider-move-up-btn';
    moveUpBtn.dataset.dividerAction = 'move-up';
    moveUpBtn.dataset.dividerId = divider.id;
    moveUpBtn.textContent = '↑';
    moveUpBtn.title = `Move this ${moveKindLabel} up with all contained characters`;
    moveUpBtn.setAttribute('aria-label', `Move ${moveKindLabel} ${moveTitleLabel} up`.trim());
    moveUpBtn.disabled = siblingMoveInfo.previous < 0;

    bindDividerAction(moveUpBtn, () => {
      moveDividerSection(divider.id, 'up');
    });

    const moveDownBtn = document.createElement('button');
    moveDownBtn.type = 'button';
    moveDownBtn.className = 'divider-action-btn divider-move-btn divider-move-down-btn';
    moveDownBtn.dataset.dividerAction = 'move-down';
    moveDownBtn.dataset.dividerId = divider.id;
    moveDownBtn.textContent = '↓';
    moveDownBtn.title = `Move this ${moveKindLabel} down with all contained characters`;
    moveDownBtn.setAttribute('aria-label', `Move ${moveKindLabel} ${moveTitleLabel} down`.trim());
    moveDownBtn.disabled = siblingMoveInfo.next < 0;

    bindDividerAction(moveDownBtn, () => {
      moveDividerSection(divider.id, 'down');
    });
    ...
    const galleryCheckBtn = document.createElement('button');
    galleryCheckBtn.type = 'button';
    galleryCheckBtn.className = 'divider-action-btn divider-gallery-check-btn';
    galleryCheckBtn.dataset.dividerAction = 'gallery-check';
    galleryCheckBtn.dataset.dividerId = divider.id;
    galleryCheckBtn.textContent = 'Gallery';
    galleryCheckBtn.title = 'Check Mudae galleries for characters inside this divider';
    galleryCheckBtn.setAttribute('aria-label', `Check Mudae galleries for divider ${divider.title || ''}`.trim());

    bindDividerAction(galleryCheckBtn, () => {
      window.MudaeGalleryUpdate?.startDividerGalleryCheck?.({
        dividerId: divider.id,
        title: divider.title || '',
        kind,
        characters: collectDividerCharacters(divider.id)
      });
    });
    ...
    actions.appendChild(galleryCheckBtn);
