...
(function(){
  ...
  function buildMixedSmSmpChunksFromNames(names, limit = DEFAULT_CHUNK_LIMIT){
    const chunks = [];
    const cleanNames = cleanNameList(names);
    if (!cleanNames.length) return chunks;
    ...
    cleanNames.forEach(name => {
      ...
      if (current !== command && candidate.length > limit) {
        ...
        const anchorName = currentNames[currentNames.length - 1] || '';
        command = '$smp ';
        currentNames = anchorName ? [anchorName] : [];
        current = anchorName ? `${command}${anchorName}` : command;

        const nextPiece = current === command ? name : `$${name}`;
        const nextCandidate = current + nextPiece;

        if (current !== command && nextCandidate.length > limit) {
          chunks.push(current);
          current = command + name;
          currentNames = [name];
          return;
