...
(() => {
  ...
  function snapshotForm() {
    ...
    return {
      ...
      galleryItems: Array.isArray(app()?.lastGalleryItems) ? app().lastGalleryItems.slice() : []
    };
  }
  ...
  function updateCharacterData(ch, data) {
    ...
    const int = internals();
    const sourceItems = Array.isArray(data.galleryItems) && data.galleryItems.length ? data.galleryItems : dedupeUrls(data.galleryUrls);
    const split = int.splitGalleryItemsBySource?.(sourceItems) || { official: [], custom: [] };
    const officialUrls = split.official?.length ? split.official.map(item => item.url) : dedupeUrls(data.galleryUrls).filter(url => !int.isAllowedCustomImageHost?.(url));
    const customUrls = split.custom?.length ? split.custom.map(item => item.url) : dedupeUrls(data.galleryUrls).filter(url => int.isAllowedCustomImageHost?.(url));
    ...
    if (split.official?.length && int.buildGalleryMetaMap) ch.mudaeImageMeta = int.buildGalleryMetaMap(split.official);

    ch.customImages = int.dedupeCustomImageUrls ? int.dedupeCustomImageUrls(customUrls) : customUrls;
    if (split.custom?.length && int.buildCustomImageMetaMap) ch.customImageMeta = int.buildCustomImageMetaMap(split.custom);
    try { int.syncCustomGalleryFlags?.(ch); } catch (_) {}
    try { int.normalizeCharacterImageGalleryPreserveOrder?.(ch); } catch (_) {}
    syncGalleryFlags(ch);
  }
  ...
  function captureSearchState() {
    const a = app();
    const e = els();
    const topValue = String(e.searchInput?.value || '');
    const floatingValue = String(e.floatingSearchInput?.value || '');
    const qValue = String(a?.filter?.q || '');
  ...
  function restoreSearchState(snapshot) {
    ...
    const raw = snapshot.top || snapshot.floating || snapshot.q || snapshot.effective || '';
    const normalized = String(raw || '').trim().toLowerCase();
    if (!normalized) return false;

    a.filter.q = normalized;
    a.filter.floatingQ = '';

    if (e.searchInput && e.searchInput.value !== raw) e.searchInput.value = raw;
    if (e.floatingSearchInput && e.floatingSearchInput.value !== raw) e.floatingSearchInput.value = raw;

    api()?.__editGalleryInternals?.syncSearchClearButton?.();
    window.MudaeFloatingBar?.syncVisibility?.();
    return true;
  }

  function restoreSearchStateSoon(snapshot) {
    if (!snapshot?.active) return;
    restoreSearchState(snapshot);
    requestAnimationFrame(() => restoreSearchState(snapshot));
    setTimeout(() => restoreSearchState(snapshot), 80);
    setTimeout(() => restoreSearchState(snapshot), 240);
  }
  ...
  function saveEditFromManager() {
    ...
    try {
      ...
      restoreSearchStateSoon(searchSnapshot);
