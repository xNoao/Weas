...
(function(){
  ...
  const state = {
    ...
    lastScrollTop: 0,
    cachedLimit: null,
    cachedColumnSetting: null,
    appliedColumnSetting: null,
    postRenderRaf: 0,
    postRenderBoard: null,
    postRenderForceAllGlass: false,
    detentRaf: 0,
    detentTimer: 0,
  ...
  function applyColumnSettingToBoard(force = false){
    const board = els().board || document.documentElement;
    const columns = state.cachedColumnSetting ?? readColumnSetting();
    state.cachedColumnSetting = columns;

    // This runs during row rebuilds and resize. Avoid repeating CSS writes when
    // the column setting has not changed; style writes can invalidate layout on
    // large boards even if the value is identical.
    if (!force && state.appliedColumnSetting === columns) {
      return columns;
    }
    ...
    state.appliedColumnSetting = columns;
    return columns;
  }

  function getColumnSetting(){
    if (state.cachedColumnSetting == null) {
      state.cachedColumnSetting = readColumnSetting();
    }
    return state.cachedColumnSetting;
  }
  ...
  function setColumnSetting(value){
    ...
    state.cachedColumnSetting = columns;
    applyColumnSettingToBoard(true);
  ...
  function getLimit(){
    if (state.cachedLimit == null) {
      state.cachedLimit = readLimit();
    }

    const limit = state.cachedLimit;
    const a = app();
    if (a && a.visibleCardLimit !== limit) a.visibleCardLimit = limit;
    return limit;
  }
  ...
  function getRowTop(rowIndex){
    const row = Math.max(0, Math.min(state.rows.length - 1, Number(rowIndex) || 0));
    return getBoardTop() + (state.rowOffsets[row] || 0);
  }

  function captureViewportRowAnchor(options = {}){
    if (!state.rows.length) return null;

    const relativeTop = Math.max(0, getScrollTop() - getBoardTop());
    const rowIndex = Math.max(0, Math.min(state.rows.length - 1, findRowByY(relativeTop)));
    const rowTop = state.rowOffsets[rowIndex] || 0;
    const row = state.rows[rowIndex];
    const rowBottom = rowTop + Math.max(1, Number(row?.height) || getCardHeight());
    const offset = Math.max(0, Math.min(rowBottom - rowTop, relativeTop - rowTop));

    return {
      rowIndex,
      offset: options.snap === true ? 0 : offset
    };
  }

  function restoreViewportRowAnchor(anchor, options = {}){
    if (!anchor || !state.rows.length) return false;

    const rowIndex = Math.max(0, Math.min(state.rows.length - 1, Number(anchor.rowIndex) || 0));
    const offset = Math.max(0, Number(anchor.offset) || 0);
    const targetTop = getRowTop(rowIndex) + offset;
    const currentTop = getScrollTop();
    const threshold = Number.isFinite(options.threshold) ? options.threshold : 3;

    if (Math.abs(currentTop - targetTop) <= threshold) return false;
  ...
  function getNearestRowDetent(options = {}){
    if (!state.rows.length) return null;

    const relativeTop = Math.max(0, getScrollTop() - getBoardTop());
    const currentRow = Math.max(0, Math.min(state.rows.length - 1, findRowByY(relativeTop)));
    const candidates = [currentRow];
    if (currentRow > 0) candidates.push(currentRow - 1);
    if (currentRow + 1 < state.rows.length) candidates.push(currentRow + 1);

    let best = null;
    let bestDistance = Infinity;

    candidates.forEach(rowIndex => {
      const rowTop = state.rowOffsets[rowIndex] || 0;
      const distance = Math.abs(relativeTop - rowTop);
    ...
    const threshold = Number.isFinite(options.threshold) ? options.threshold : 64;
  ...
  function settleToNearestRowDetent(options = {}){
    if (isGlassMode() || getLimit() <= 0 || !state.rows.length) return false;
    if (state.stableFocusMode) return false;

    const detent = getNearestRowDetent(options);
    if (!detent) return false;

    if (Math.abs(getScrollTop() - detent.top) <= 2) return false;

    window.scrollTo({ top: Math.max(0, detent.top), behavior: 'auto' });
    state.lastScrollTop = getScrollTop();
  ...
  function scheduleRowDetent(options = {}){
    if (state.detentRaf) {
      cancelAnimationFrame(state.detentRaf);
      state.detentRaf = 0;
    }
    if (state.detentTimer) {
      clearTimeout(state.detentTimer);
      state.detentTimer = 0;
    }

    const delay = Number.isFinite(options.delay) ? options.delay : 90;

    state.detentTimer = setTimeout(() => {
      state.detentTimer = 0;
      state.detentRaf = requestAnimationFrame(() => {
        state.detentRaf = 0;
        settleToNearestRowDetent(options);
      });
    }, delay);
  }

  function markUserScroll(delta){
    if (!Number.isFinite(delta) || Math.abs(delta) < 1) return;

    const now = performance.now();
    state.lastUserScrollDirection = delta > 0 ? 1 : -1;
    state.userScrollActiveUntil = now + 220;
  }

  function isUserScrollActive(){
    return performance.now() < Number(state.userScrollActiveUntil || 0);
  }

  function isEdgeKeyNavigationActive(){
    return performance.now() < Number(state.edgeKeyNavUntil || 0);
  }

  function shouldPreserveScrollDuringWindowSwap(){
    if (isGlassMode()) return false;
    if (!state.rows.length) return false;
    if (state.stableFocusMode) return false;
    if (isEdgeKeyNavigationActive()) return false;
  ...
  function calcColumns(metricsOverride = null){
    ...
    const metrics = metricsOverride || getGridMetrics();
  ...
  function rebuildRows(){
    ...
    const columns = calcColumns(metrics);
  ...
  function schedulePostRender(board, options = {}){
    state.postRenderBoard = board || els().board || state.postRenderBoard;
    state.postRenderForceAllGlass = state.postRenderForceAllGlass || options.forceLoadAllGlass === true;

    if (state.postRenderRaf) return;

    state.postRenderRaf = requestAnimationFrame(() => {
      state.postRenderRaf = 0;
      const target = state.postRenderBoard || els().board;
      const forceLoadAllGlass = state.postRenderForceAllGlass;
      state.postRenderBoard = null;
      state.postRenderForceAllGlass = false;

      if (!target) return;

      window.MudaeMinimalImageLoader?.releaseVisible?.(target, {
        ahead: 2400,
        behind: 360,
        max: 220,
        priority: true
      ...
      if (forceLoadAllGlass) {
        setTimeout(() => window.MudaeMinimalImageLoader?.forceLoadAll?.(target), 60);
      }
  ...
  function renderAllRows(){
    ...
    schedulePostRender(board, { forceLoadAllGlass: isGlassMode() });
  ...
  function renderWindow(force = false){
    ...
    const preserveScroll = shouldPreserveScrollDuringWindowSwap();
    const scrollBeforeSwap = preserveScroll ? getScrollTop() : 0;
    ...
    if (preserveScroll) {
      preserveScrollPosition(scrollBeforeSwap);
    }
  ...
  function bind(){
    ...
    const flushScrollUpdate = () => {
      state.scrollUpdateRaf = 0;

      const currentScrollTop = getScrollTop();
      const viewportHeight = getViewportHeight();
      const delta = currentScrollTop - state.lastScrollTop;
      const isLargeJump = Math.abs(delta) > Math.max(900, viewportHeight * 1.20);

      if (!isLargeJump && !isEdgeKeyNavigationActive()) {
        markUserScroll(delta);
      }

      state.lastScrollTop = currentScrollTop;

      // Keyboard End/Home and scrollbar thumb jumps can move several virtual
      // windows in one frame. Treat those as hard jumps so the mounted row
      // window is recalculated immediately instead of waiting until the user
    ...
    const update = () => {
      if (state.scrollUpdateRaf) return;
      state.scrollUpdateRaf = requestAnimationFrame(flushScrollUpdate);
    };
    ...
    const rebuildForResize = () => {
      ...
      const anchor = captureViewportRowAnchor();
      ...
      if (anchor) {
        requestAnimationFrame(() => restoreViewportRowAnchor(anchor, { threshold: 8 }));
      }
    };

    const scheduleResizeRebuild = () => {
      if (!state.entries.length) return;
    ...
    window.addEventListener('resize', scheduleResizeRebuild, { passive: true });
    ...
    window.addEventListener('keydown', event => {
      ...
      if (['Home','End','PageDown','PageUp','ArrowDown','ArrowUp',' '].includes(event.key)) {
        if (['Home','End'].includes(event.key)) {
          state.edgeKeyNavUntil = performance.now() + 900;
        } else {
          state.userScrollActiveUntil = performance.now() + 260;
          state.lastUserScrollDirection = ['PageUp','ArrowUp'].includes(event.key) ? -1 : 1;
        }
        ...
        // Soft row detents are useful for PageUp/PageDown and arrow/space
        // navigation, where the browser can land between virtual rows. Avoid
        // Home/End here so their stable behavior remains exactly as in the
        // v2.720 base.
        if (!['Home','End'].includes(event.key)) {
          scheduleRowDetent({ delay: 130, threshold: 72 });
        }
      }
    }, true);
  }
  ...
  function moveSourceToTarget(targetId, placement = null){
    ...
    const actualPlacement = placement === 'after' || placement === 'before' ? placement : null;

    return api()?.moveCharacterRelativeToTargetDataOnly?.(sourceId, targetId, actualPlacement) || false;
  }
...
renderWindow,
    ...
    flushSave,
    cancelFocus,
    settleToNearestRow: settleToNearestRowDetent,
    focusCharacterById
  };
})();
