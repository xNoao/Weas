/* Mudae Organizer Rebuild move utilities.
   Keeps low-level state reordering helpers out of app.js.
   This module does not render UI; callers decide how/when to refresh the board.
*/
(function(){
  'use strict';
  ...
  function create(context){
    ...
    function moveCharacterToRealDisplayPositionById(sourceId, targetPosition){
      ...
      // Treat targetPosition as the desired FINAL display position. Resolve it
      // after source removal so adjacent moves like #768 -> #769 are valid.
      const [item] = app.state.characters.splice(sourceIndex, 1);
