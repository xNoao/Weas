/* Mudae Organizer Rebuild - exports module v2.100-clean-exportzone
   - Global export: chunk 1 $sm, following chunks $smp
   - Nitro toggle 2000/4000
   - Hover overlay sidebar
   - Changed exports: max 5 move history, small zones merged when nearby
   - Full export: dropdown/window opening downward
*/
(function(){
  'use strict';
  ...
  function closeExportSidebar(){
    ...
    if (panel) {
      // Keep the panel mounted/offscreen instead of waiting for hidden=true.
      // This avoids the rapid hover/leave race where a pending hide fires after reopen.
      panel.hidden = false;
  ...
  function renderExportSidebar(){
    ...
    chunks.forEach((chunk, index) => {
      ...
      row.className = 'export-sidebar-chunk export-menu-chunk is-compact-copy';
      row.innerHTML = `
        <header class="export-chunk-head export-sidebar-chunk-head">
          ...
          <button class="mhp-menu-btn export-neutral-btn export-sidebar-copy-one" type="button" title="Copy this ${index === 0 ? '$sm' : '$smp'} chunk">Copy</button>
        </header>
        <input class="export-sidebar-text export-menu-command export-sidebar-full-command" readonly value="${escapeHtml(chunk)}" title="Click to copy this ${index === 0 ? '$sm' : '$smp'} chunk">
      `;
      ...
      row.title = `Click to copy ${index === 0 ? '$sm' : '$smp'} chunk ${index + 1}`;
      row.addEventListener('click', event => {
        event.preventDefault();
        copyChunk();
      });
      const copyButton = row.querySelector('.export-sidebar-copy-one');
      if (copyButton) {
  ...
  function openExportSidebar(){
    ...
    const existingPanel = document.getElementById('exportSidebar');
    const wasAlreadyShowing = !!existingPanel && !existingPanel.hidden && !existingPanel.classList.contains('is-closing');

    // renderExportSidebar() recomputes export chunks for the whole roster and
    // rebuilds every row's DOM/innerHTML/listeners from scratch — it is not
    // cheap. openExportSidebar() runs on every hotspot mouseenter, and the
    // mouse grazing the hotspot's edge (e.g. while scrolling) can fire that
    // repeatedly in quick succession. Previously each one did a full rebuild,
    // which is what caused the visible flicker/slowness in this area. If the
    // panel is already open and showing current content, there is nothing new
    // to render — just keep it open. A genuinely fresh open (panel was hidden
    // or mid-close) still gets a full render, so content is never stale.
    const panel = wasAlreadyShowing ? existingPanel : renderExportSidebar();
    ...
    const ok = await c.showAppDialog({
      ...
      renderContent(content) {
        ...
        chunks.forEach((chunk, index) => {
          ...
          const copyButton = row.querySelector('.export-copy-one');
          if (copyButton) {
