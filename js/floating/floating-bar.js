...
(function(){
  ...
  function queueRender(){
    ...
    app.searchTimer = setTimeout(() => {
      ...
      if (typeof bridge.scheduleBoardRender === 'function') {
        bridge.scheduleBoardRender('floating-search-input', { delay: 0 });
      } else {
        bridge.renderBoard();
      }
    }, 120);
  ...
  function dispatchSearchFromFloating(value, options = {}){
    ...
    if (rawValue.trim() && typeof bridge?.getMultiSelectMode === 'function' && bridge.getMultiSelectMode()) {
      bridge.toggleMultiSelectMode?.(false);
    }
    ...
    if (!main) {
      const nextNormalized = rawValue.trim().toLowerCase();
      if (nextNormalized === activeSearch.trim().toLowerCase()) {
        bridge?.setUnifiedSearchValue?.(rawValue, { updateFloating: false, captureSearchOrigin: false });
        syncFloatingInputValue(rawValue);
        return;
      }
    ...
    const nextNormalized = rawValue.trim().toLowerCase();
    const activeNormalized = activeSearch.trim().toLowerCase();
    const mainNormalized = String(main.value || '').trim().toLowerCase();

    // Scroll-driven floating-bar visibility can sync the same query back into
    // the top search. Treat that as a no-op so it cannot schedule a delayed
    // filtered-board rebuild and push the user back to the top of results.
    if (nextNormalized && nextNormalized === activeNormalized && mainNormalized === nextNormalized) {
      syncFloatingInputValue(rawValue);
      return;
    }
  ...
  function syncActionButtons(){
    ...
    if (moveSelected) {
      ...
      moveSelected.title = targetMode
        ? 'Click a destination card. Press again to cancel.'
        : 'Move selected characters to an exact position. Hold Shift/Ctrl for click-destination mode.';
    }
  ...
  function bindActionButtons(){
    ...
    bindOnce(getEl('floatingMoveSelectedBtn'), 'bound', event => {
      const bridge = api();
      if (event?.shiftKey || event?.ctrlKey || event?.metaKey || event?.altKey) {
        bridge?.setMultiMoveTargetMode?.();
      } else if (typeof bridge?.promptMoveSelectedCharactersSafe === 'function') {
        bridge.promptMoveSelectedCharactersSafe();
      } else {
        bridge?.setMultiMoveTargetMode?.();
      }
      syncActionButtons();
  ...
  function bind(){
    ...
    if (backTop && !backTop.dataset.bound) {
      ...
      const goTop = event => {
        ...
        event?.stopPropagation?.();
        const bridge = api();
        try { bridge?.clearSearchNavigationSideEffects?.(); } catch {}
        try { window.MudaeBoardController?.cancelFocus?.(); } catch {}
        try { window.MudaeBoardController?.clearSelection?.(); } catch {}
      ...
      backTop.addEventListener('pointerdown', goTop);
